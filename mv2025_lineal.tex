...
We denote by $a(\cdot, \cdot):H_0^1(\om) \times H_0^1(\om)\to \reals$  the bilinear form associated to the second-order differential operator $A$, defined by $$a(y,v)=\int_\om a_{ij}(x)\frac{\partial y(x)}{\partial x_j} \frac{\partial v(x)}{\partial x_i} dx + \int_\om c_0(x)\,y(x)v(x)\,dx, $$
while $a^*$ denotes the corresponding bilinear operator associated to $A^*$.
	
	It is well known from the Lax–Milgram theorem and standard elliptic regularity estimates \cite{brezis2010}, we know that for every $w \in L^2(\om)$ there exist $y\in H_0^1(\om)$ and a positive constant $c_a>0$, satisfying: 
	\begin{subequations}\label{eq:state_formul}
		\begin{align}
			&a(y,v) = (w,v)_{L^2(\om)}, \quad \forall v \in H_0^1(\om),\label{eq:state}\\
			&\norm{y}_{H_0^1(\om)} \leq c_a\norm{w}_{L^2(\om)}. \label{eq:state_continuity}
		\end{align}
	\end{subequations}
 The same is true for the adjoint problem:
	\begin{subequations}\label{eq:adj_eq}
		\begin{align}
			&a^*(z,v) = (w,v)_{L^2(\om)}, \quad \forall v \in H_0^1(\om),\label{eq:state*}\\
			&\norm{z}_{H_0^1(\om)} \leq c_a^*\norm{w}_{L^2(\om)}\label{eq:esti_state*},
		\end{align}
	\end{subequations}
for some positive constant $c_a^*$.  

Furthermore, thanks to Theorem 4.4 in \cite{stampacchia65} and the continuous injection $L^2(\om) \hookrightarrow W^{-1,p}(\om)$ for a fixed $p>n/2$, equation \eqref{eq:state_formul} admits a unique solution in $C(\bar\om)\cap H_0^1(\om)$. We denote the linear and continuous operator $S:L^2(\om)\to L^{\infty}(\om)\cap H_0^1(\om)$, which assigns any $w\in L^2(\om)$  the corresponding state $y$  satisfying \eqref{eq:state} with $w$ as right-hand side. The range of $S$ in $L^\infty(\om)$ complies better in our later discussions. The continuity of $S$ also implies the existence of a constant $c_{\infty}>0$ such that the solution $y=Sw$ of \eqref{eq:state} satisfies
\begin{equation}\label{eq:esti_state_uniform}
    \norm{y}_{L^{\infty}(\om)}\le c_{\infty}\norm{w}_{L^2(\om),}
\end{equation}
Analogously, for the solution $z$ of \eqref{eq:adj_eq} it holds that 
\begin{equation}\label{eq:esti_state*_uniform}
    \norm{z}_{L^{\infty}(\om)}\le c_{\infty}^*\norm{w}_{L^2(\om),}
\end{equation}

for some constant $c_{\infty}^*>0$. We denote by $S^*:L^2(\om)\to L^{\infty}(\om)\cap H_0^1(\om)$ the corresponding solution  operator that for any $w\in L^2(\om)$ assigns the solution $z=S^*(w)$ of \eqref{eq:adj_eq}. 

Given a control $  u \in L^2(\om)$ with associated state $y=Su \in H_0^1(\om)$, the adjoint equation consists of the following variational problem: 
	\begin{align}\label{eq:costate}
		&a^*(\varphi,v) = (y_d-y,v)_{L^2(\om)}, \quad \forall v \in H_0^1(\om)
	\end{align}	
This equation fits the formulation of \eqref{eq:adj_eq}. Moreover, $H^2(\om)$ regularity for the states $y$ and $\varphi$ can be achieved provided that $\om$ is a convex domain and the coefficients $a_{ij}$ are Lipschitz continuous \cite[Theorem 3.2.1.2]{gri85}. Moreover, we define a Carathéodory function $L:\om\times\reals\to \reals$ of a class $C^2$ respect to the second variable, that satisfy the following: 
...
Let us review the optimality conditions of a local solution $\bar u$ for \eqref{eq:P}. Let $\bar y$ the associated state. The authors in \cite{itokun2014} derived necessary optimality conditions for optimal controls by applying a Pontryagin's maximum principle. For this, the Hamiltonian associated to \eqref{eq:P} see \cite{itokun2014,wachsmuth19,caswach20} was defined by $H:\om\times\mathbb{R}^3\to\mathbb{R}$, 
\[
...
\begin{subequations}\label{eq:optimal_system_P0}
	\begin{align}
&\begin{array}{rlc}
	A\bar y  & = \bar u & \text{ in } \om, \\
      \bar y &= 0 & \text{ on } \ga, 
\end{array}\\
%
&\,\begin{array}{rlc}
	A^*\varphi & = \dfrac{\partial L}{\partial y}(\cdot,\bar y)& \text{ in } \om, \\
      \varphi &= 0 & \text{ on } \ga ,
...
\begin{subequations}\label{eq:optimal_system_changed_variable}
	\begin{align}
&\begin{array}{rlc}
	A\bar y & = \hat u + \jmath_{\alpha,\beta}\sign(\hat u), & \text{ in } \om, \\
      \bar y &= 0, & \text{ on } \ga, 
...
These formulas allow us to write the optimality system \eqref{eq:optimal_system_changed_variable} in the form $\mathbf{\Phi}(\hat u) =0$. First, we require to express $\varphi$ and $\zeta$ as functions of $\hat u$. Defining $G = T \circ S \circ R$  we can write $\varphi = G(u)$. Proceeding as in \cite{CasasMateos2024}, we use \eqref{eq:proj_u} and \eqref{eq:projection_l1} to define the function $\psi:\reals \to \reals$ given by

\begin{equation}\label{eq:psi}   
\psi(t)= \proj_{[-\delta+\jmath_{\alpha,\beta}, \delta-\jmath_{\alpha, \beta}]}\l(-\frac{1}{\alpha}\l[t+\proj_{[-\omega_{\alpha,\beta},\omega_{\alpha,\beta}]}(-t)\r]\r),
\end{equation}
...
\begin{subequations}\label{eq:reg_system}
	\begin{align}
&\begin{array}{rlc}
	A y  & = \hat u + \jmath_{\alpha,\beta}\sign_\epsilon(\hat u), & \text{ in } \om, \\
     y &= 0, & \text{ on } \ga, 
\end{array}\label{eq:optimal_system_P0_a}\\
%
&\,\begin{array}{rlc}
	A^*\varphi & = \dfrac{\partial L}{\partial y}(\cdot, y),& \text{ in } \om, \\
      \varphi &= 0, & \text{ on } \ga ,
\end{array} \label{eq:optimal_system_P0_b}\\
&\hat u(x)=\ds
\begin{cases} 
...
\end{equation}
and if $N\in \partial_{G_{\epsilon}}(u)$, then $N$ is a linear operator from $L^2(\om)$ to itself such that, for any $v\in L^2(\om)$, there exists two measurable functions $h, \tilde{h}$ such that $h(x)\in \partial^{\circ}\psi(G_{\epsilon}(u)(x))$, $\tilde{h}(x)\in \partial^{\circ}\sign_{\epsilon}(u(x))$, and
...
\end{lemma}
\begin{proof}
The Clarke subdifferentiability of $\psi$, defined in \eqref{eq:psi}, was sutided in \cite{CasasMateos2024} for different interval values. The corresponding derivative is given by:
    \[
    \partial^{\circ}\psi(t)=\begin{cases}
        \{0\}, & t\in (-\infty, -\alpha\delta)\cup (-\omega_{\alpha,\beta}, \omega_{\alpha,\beta})\cup (\alpha\delta,+\infty),\\
        \l\{-\frac{1}{\alpha}\r\}, & t \in (-\alpha \delta,-\omega_{\alpha,\beta})\cup (\omega_{\alpha,\beta}, \alpha \delta),\\
        \l[-\frac{1}{\alpha},0\r], & t\in \l\{-\alpha\delta, -\omega_{\alpha,\beta}, \omega_{\alpha,\beta}, \alpha\delta\r\}.
    \end{cases}
    \]
...
where
\[
J_1 = \{x\in \om: -\alpha\delta\le t \le -\omega_{\alpha,\beta}\},
\]
\[
J_2 = \{x\in \om: \omega_{\alpha,\beta}\le t\le  \alpha\delta\},
\]
\[
J_3 = \{x\in \om: |t|\le \epsilon\}.
\]
\begin{theorem}
    Let $u^0$ a sufficient close point of the solution of \eqref{eq:Phi_eps=0}. Then, the iterations $(u_k)_{k\in\mathbb{N}}$ generated through Algorithm 1 converges superlinearly to the solution $\hat{u}$ of \eqref{eq:Phi_eps=0}. 
\end{theorem}
...
&\begin{array}{rlc}\label{eq:KKT_1}
	A\hat y  & = \hat u + \jmath_{\alpha,\beta}\sign_\epsilon(\hat u), & \text{ in } \om, \\
      \hat y &= 0, & \text{ on } \ga,  
\end{array}\\
%
&\begin{array}{rlc}\label{eq:KKT_2}
	A^*\varphi  & = \dfrac{\partial L}{\partial y}(\cdot,\hat y),& \text{ in } \om, \\
      \varphi &= 0, & \text{ on } \ga ,
\end{array} \\
